...
(defn replace-mentions
  ([text users]
   (replace-mentions text users 0))
  ([text users idx]
   (let [mention-key-idx (string/index-of text "@" idx)]
     (if-not mention-key-idx
       text
       (let [{:keys [public-key alias]} (match-mention text users mention-key-idx)]
         (if-not alias
           (recur text users (inc mention-key-idx))
           (let [new-text (string/join
             ...
             (recur new-text users mention-end))))))))
